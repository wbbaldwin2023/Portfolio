...
public class Tester {
	...
	public static int NUM_TESTS = 5000;  //5000
	...
	private static <T> boolean compareLists(java.util.LinkedList<T> list1, list.LinkedList<T> list2) {
		...
		//check forwards
		while(!it2.isPastEnd() && it1.hasNext()) {
		...
		it2 = list2.back();
		while(!it2.isPastBeginning() && it1.hasNext()) {
			if(!it1.next().equals(it2.value())) return false;
			it2.moveBackward();
		}
		return true;
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<NUM_TESTS; i++) {
			...
			//System.out.println("inserting " + rand + "  at " + ind + "  stuList " + studList.toString());
			if(!compareLists(compList, studList)) {
				System.out.println("ERROR: Inserting at generic index (i.e, insertAt()) failed");
				System.exit(1);
			}
		}
		System.out.println("studList is: " + studList.toString() + " /ncompList is:  " + compList.toString());
		System.out.println("Done. Looks good");
		...
		for(int i=0; i<NUM_TESTS; i++) {
			...
			//System.out.println("find returns " + ind2 + "  compList find" + ind1);
			//System.out.println("studlist is: " + studList.toString());
			if(!(ind1 == ind2)) {
		...
		for(int i=0; i<NUM_TESTS; i++) {
			...
			//System.out.println("get returns " + ind2 + "  compList get" + ind1);
			if(!(ind1 == ind2)) {
		...
		for(int i=compList.size()-2; i>0; i--) {
			...
			studIt.moveBackward();
			//System.out.println("studlist is: " + studList.toString()  + "complist is: " + compList.toString());
			if(!(compareLists(compList, studList))) {
